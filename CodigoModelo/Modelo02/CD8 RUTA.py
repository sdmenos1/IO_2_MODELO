# vrp_cd8_print_fixed.py 
# ORS Matrix + MILP (PuLP) para CD8 — Corrección de nombres de variables
import os
import math
import time
import requests
...
rows = [
    ("C81",   201, -12.235258, -76.933109),
    ("C95",   265, -12.239422, -76.933756),
    ("C128",  265, -12.237475, -76.930448),
    ("C134",  263, -12.23514 , -76.933393),
]

# coordenadas exactas del CD8
CD8_LAT = -12.239124
CD8_LON = -76.928217
...
# ---------------------------
nodes = []
nodes.append({"id":"CD8","q":0,"lat":CD8_LAT,"lon":CD8_LON})
for r in rows:
...
for idx, (k, route) in enumerate(routes, start=1):
    ...
    # inicio
    print(f" CD8 (Carga: {cum_load}, Tiempo: {cum_time} seg)", end="")
    ...
    for pos in range(len(route)-1):
        ...
        if j != 0:
            ...
            print(f" -> CD8 (Llegada+Recarga. Tiempo acum: {cum_time} seg)", end="")
...
# ---------------------------
m = folium.Map(location=[CD8_LAT, CD8_LON], zoom_start=14)
folium.Marker([CD8_LAT, CD8_LON], popup="CD8", icon=folium.Icon(color="red", icon="truck")).add_to(m)
for n in nodes[1:]:
