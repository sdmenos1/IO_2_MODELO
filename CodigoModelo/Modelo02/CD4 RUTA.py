# vrp_cd4_print_fixed.py 
# ORS Matrix + MILP (PuLP) para CD4 — Corrección de nombres de variables
import os
import math
import time
import requests
...
rows = [
    ("C65",  201, -12.187621, -76.953027),
    ("C68",  465, -12.190014, -76.953278),
    ("C70",  465, -12.189841, -76.951293),
    ("C71",  402, -12.190125, -76.946186),
    ("C85",  265, -12.191450, -76.953361),
    ("C91",  328, -12.187344, -76.950333),
    ("C93",  201, -12.190354, -76.947089),
    ("C102", 465, -12.191919, -76.950556),
    ("C105", 465, -12.186543, -76.947946),
    ("C106", 340, -12.186777, -76.949135),
    ("C110", 201, -12.191502, -76.955888),
    ("C111", 265, -12.192858, -76.945235),
    ("C113", 327, -12.187764, -76.942859),
    ("C118", 327, -12.188455, -76.944263),
    ("C120", 465, -12.186005, -76.951421),
    ("C123", 263, -12.188438, -76.950117),
    ("C133", 402, -12.187638, -76.953293),
    ("C138", 201, -12.189818, -76.948092),
    ("C139", 201, -12.190553, -76.948687),
    ("C145", 340, -12.189872, -76.941287),
    ("C149", 201, -12.192577, -76.949849),
]

# coordenadas exactas del CD4
CD4_LAT = -12.189898
CD4_LON = -76.951337
...
# ---------------------------
nodes = []
nodes.append({"id":"CD4","q":0,"lat":CD4_LAT,"lon":CD4_LON})
for r in rows:
...
for idx, (k, route) in enumerate(routes, start=1):
    ...
    # inicio
    print(f" CD4 (Carga: {cum_load}, Tiempo: {cum_time} seg)", end="")
    ...
    for pos in range(len(route)-1):
        ...
        if j != 0:
            ...
            print(f" -> CD4 (Llegada+Recarga. Tiempo acum: {cum_time} seg)", end="")
...
# ---------------------------
m = folium.Map(location=[CD4_LAT, CD4_LON], zoom_start=14)
folium.Marker([CD4_LAT, CD4_LON], popup="CD4", icon=folium.Icon(color="red", icon="truck")).add_to(m)
for n in nodes[1:]:
