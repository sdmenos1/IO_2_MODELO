# vrp_cd7_print_fixed.py 
# ORS Matrix + MILP (PuLP) para CD7 — Corrección de nombres de variables
import os
import math
import time
import requests
...
rows = [
    ("C1",   201, -12.228684, -76.946888),
    ("C38",  465, -12.222848, -76.943664),
    ("C45",  327, -12.225223, -76.942540),
    ("C69",  327, -12.238410, -76.936864),
    ("C73",  263, -12.235437, -76.943783),
    ("C74",  263, -12.234152, -76.937544),
    ("C78",  264, -12.238942, -76.936457),
    ("C83",  328, -12.233129, -76.943475),
    ("C90",  327, -12.227396, -76.939568),
    ("C92",  263, -12.230167, -76.941201),
    ("C96",  327, -12.235609, -76.936261),
    ("C97",  327, -12.238262, -76.935386),
    ("C100", 265, -12.236743, -76.939965),
    ("C103", 265, -12.232100, -76.949524),
    ("C107", 340, -12.236453, -76.934578),
    ("C108", 327, -12.230456, -76.943312),
    ("C109", 327, -12.236650, -76.942244),
    ("C117", 263, -12.233179, -76.938892),
    ("C126", 402, -12.232534, -76.947539),
    ("C137", 327, -12.228851, -76.940547),
    ("C142", 327, -12.233834, -76.947255),
    ("C144", 327, -12.229842, -76.939933),
    ("C148", 340, -12.229651, -76.946397),
]

# coordenadas exactas del CD7
CD7_LAT = -12.233326
CD7_LON = -76.938849
...
# ---------------------------
nodes = []
nodes.append({"id":"CD7","q":0,"lat":CD7_LAT,"lon":CD7_LON})
for r in rows:
...
solver = pulp.PULP_CBC_CMD(msg=1, timeLimit=900)
...
for idx, (k, route) in enumerate(routes, start=1):
    ...
    # inicio
    print(f" CD7 (Carga: {cum_load}, Tiempo: {cum_time} seg)", end="")
    ...
    for pos in range(len(route)-1):
        ...
        if j != 0:
            ...
            print(f" -> CD7 (Llegada+Recarga. Tiempo acum: {cum_time} seg)", end="")
...
# ---------------------------
m = folium.Map(location=[CD7_LAT, CD7_LON], zoom_start=14)
folium.Marker([CD7_LAT, CD7_LON], popup="CD7", icon=folium.Icon(color="red", icon="truck")).add_to(m)
for n in nodes[1:]:
